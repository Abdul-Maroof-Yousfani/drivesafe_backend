...
function getStateByCoordinates(lat, lng, cityName) {
  ...
  if (lat >= 29 && lat <= 33 && lng >= 70 && lng <= 75) {
    if (lat >= 33.5 && lat <= 34 && lng >= 72.8 && lng <= 73.2) return 'FANA';
    if (lat >= 33.5 && lng < 72) return 'KPK';
    if (lat >= 33.5 && lng >= 73.5) return 'AZAD KASHMIR';
    return 'PUNJAB';
  }
  ...
  if (lat >= 31 && lat <= 36 && lng >= 70 && lng <= 74) {
    if (lat < 33.5 || (lat >= 33.5 && lng < 72)) {
      if (lat >= 33 && lat <= 35 && lng >= 70 && lng <= 71.5) return 'FATA';
      return 'KPK';
    }
    if (lat >= 33.5 && lng >= 73.5) return 'AZAD KASHMIR';
  }
...
async function seedCities(prisma) {
  ...
  const citiesPath = path.join(process.cwd(), 'city.json');
  let citiesData = [];
  try {
     citiesData = JSON.parse(fs.readFileSync(citiesPath, 'utf-8'));
  } catch(e) {
      console.warn('⚠️ city.json not found, skipping cities');
      return { statesCreated, statesSkipped, citiesCreated: 0, citiesSkipped: 0 };
  }
  ...
  for (const city of pakistanCities) {
    try {
      ...
      const stateName = getStateByCoordinates(parseFloat(city.lat), parseFloat(city.lng), cityName);
